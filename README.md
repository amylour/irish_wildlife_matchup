...
| 8. | Play Again button responds to second click only | Win Modal | Resolved | 'onClick' placed in wrong HTML element and playAgain function updated to be called on first click by adding an EventListener to 'win-button' ID and adding the playAgain() function to it outside of the original function.
| 9. | Audio not playing onload | Audio | Resolved | I added an if/else statement to audio play function to control when the music is muted/playing. |
| 10. | Start Modal will appear again if user rapidly clicks on 'Tick' button when window first loads |
